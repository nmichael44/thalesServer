  ...
  private val mockHttpApp: HttpApp[IO] = HttpApp[IO]:
    ...
    case req @ GET -> Root / "user" =>
      ...
      if authHeader == "Bearer secret-token" then
        Ok(writeToString(testUser))
      else
        Forbidden("Unauthorized user request")

    case req @ POST -> Root / "echo" =>
      req.as[String].flatMap: bodyStr =>
  ...
  "ExternalApiClientServiceLive" - {
    ...
    "fetchUri" - {
      "should fetch plain text successfully and include optional headers" in {
        ...
        apiClient.fetchUri(uri, customHeaders).map: response =>
      ...
      "should fail when HTTP status is not successful" in {
        val uri = uri"/error"
        apiClient.fetchUri(uri).attempt.map:
          case Left(err) =>
    ...
    "getAs" - {
      "should decode typed JSON values successfully with required headers" in {
        ...
        apiClient.getAs[TestUser](uri, authHeaders).map: user =>
          assert(user == testUser)
      }

      "should fail when authentication headers are missing" in {
        val uri = uri"/user"
        apiClient.getAs[TestUser](uri).attempt.map:
          case Left(err) =>
    ...
    "postAs" - {
      "should encode request body and decode response body successfully" in {
        ...
        apiClient.postAs[TestRequest, TestResponse](uri, requestPayload).map: response =>
      ...
      "should propagate server failures" in {
        ...
        apiClient.postAs[TestRequest, TestResponse](uri, requestPayload).attempt.map:
          case Left(err) =>
            assert(err.getMessage.contains("failed with status: 500"))
          case Right(_) =>
